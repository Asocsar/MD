...
#Max Power 
#l <- which(df[var_numericas]$Max_Power <4)
#df[var_numericas][l, "Max_Power"] <- NA

#L2_Cache
#l <-which(df[var_numericas]$L2_Cache == 0)
#df[var_numericas][l, "L2_Cache"] <- NA 
...
# df_selected <- df_selected[complete.cases(df_selected[, "Memory" ] ), ]
df_selected <- df_selected[complete.cases(df_selected[, "TMUs" ] ), ]
df_selected <- df_selected[complete.cases(df_selected[, "Texture_Rate" ] ), ]
df_selected
...
for (k in var_numericas) {
  l <- sum(is.na(df_selected[var_numericas][,k]))
  print(k)
  print(l)
}
dim(df_selected)
