...
df[var_numericas]$Release_Date <- sub("\n", "", df[var_numericas]$Release_Date)
df[var_numericas]$Release_Date <- gsub(" ", "", df[var_numericas]$Release_Date)
...
df[var_cualitativas] <- gsub("*\\.[0]+", "", df[var_cualitativas]$Direct_X)
...
```{r}
unique(df_selected$Architecture)
```
...
```{r}
sort(df_selected$L2_Cache, decreasing = TRUE)
df_selected[df_selected$Architecture == "G92 (x2) " ,]
```
